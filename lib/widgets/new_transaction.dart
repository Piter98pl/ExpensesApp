...
class _NewTransactionState extends State<NewTransaction> {
  ...
  void _submitData() {
    ...
    widget.addTransaction(
      // thanks to widget. i can access poperty addTransaction which has this function reference
      // i get even though i'm tehnically in different class
      enteredTitle,
      enteredAmount,
      _selectedDate,
    );
    ...
    //and context here is a special property which is available class wide in state class here even though
    //you never defined a property named context
...
//showDatePicker also needs to know the context of the widget in which you're calling it,
//context here on the right refers to that class property context
    ).then((pickedDate) {
      if (pickedDate == null) {
        // if true, then user pressed cancel and then we just return in this anonymous function and
        //not continue with any other code
        return;
      }
      setState(() {
    ...
    // then() simply allows us to provide a function which is executed once the Future
    //resolves(rozstrzygać, zadecydować) to a value, so once the user chose a date
  }
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    AdaptiveFlatButton('Wybierz datę', _presentDatePicker)
                  ],
                ),
              ),
              RaisedButton(
