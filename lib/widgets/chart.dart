...
class Chart extends StatelessWidget {
  ...
  List<Map<String, Object>> get groupedTransactionsValues {
    ...
    return List.generate(7, (index) {
      //we use list and then generate which is a utility constructor on the list class,
      //List is a core Dart object, class
      //and it has a generate constructor which generates us a new list,
      // in this constructor we define length and then function which will be called for every element,
      //it executes this function for every generated list element with index being 0,1,2,3,4,5,6,
      final weekDay = DateTime.now().subtract(Duration(days: index));
      //So for the first round when index is zero, we have duration of zero days
      //so we subtract zero days which means we have still datetime now. in the next round,
      //for the next list item, which is generated index is one, so we subtract one day,
